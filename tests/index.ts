...
import { MySQLDriver } from "../src/index";
import { readFileSync } from 'fs';
import { Person } from './models/person';
import { Dummy } from './models/dummy';

const dataSource: DataSource = MySQLDriver.createDataSource(JSON.parse(readFileSync(path.resolve(__dirname, "test.config.json")).toString()));
const driver: MySQLDriver = <MySQLDriver>dataSource.driver;

driver.debug = console.log;
...
describe("MySQL tsbean-orm driver testing", () => {
    ...
    before(async () => {
        ...
        const script = readFileSync(path.resolve(__dirname, "test.sql")).toString();
        const sentences = script.split("\n").filter(a => !!a).map(a => {
            return a.replace(/[\r\n]+/g, "").replace(/\-\-.*$/g, ""); // Remove comments
        }).join("").split(";").filter(a => !!a);

        for (let sentence of sentences) {
            await driver.customQuery(sentence, []);
        }
    });
