...
import { normalizeSQLResults, toSnakeCase, toSQLCompatibleValue } from "./utils";
...
export class MySQLDriver implements DataSourceDriver {
    ...
    findByKey(table: string, keyName: string, keyValue: any): Promise<GenericRow> {
        const sentence = "SELECT * FROM `" + table + "` WHERE `" + toSnakeCase(keyName) + "` = ?";
        ...
        return new Promise<any>(function (resolve, reject) {
            this.pool.query(sentence, values, function (error, results, fields) {
                ...
                if (results && results.length > 0) {
                    resolve(normalizeSQLResults(results)[0]);
                } else {
                    resolve(null);
                }
            }.bind(this));
    ...
    private generateSelectSentence(table: string, filter: GenericFilter, sortBy: string, sortDir: SortDirection, skip: number, limit: number, projection: Set<string>): { sql: string, values: any[] } {
        ...
        const cond1 = filterToSQL(filter);
        ...
        if (sortBy) {
            sentence += " ORDER BY `" + toSnakeCase(sortBy) + "` " + (sortDir === "desc" ? "DESC" : "ASC");
        }
    ...
    find(table: string, filter: GenericFilter, sortBy: string, sortDir: SortDirection, skip: number, limit: number, projection: Set<string>): Promise<GenericRow[]> {
        ...
        return new Promise<any[]>(function (resolve, reject) {
            this.pool.query(sentence, values, function (error, results, fields) {
                ...
                if (results) {
                    resolve(normalizeSQLResults(results));
                } else {
                    resolve([]);
                }
            }.bind(this));
    ...
    findStream(table: string, filter: GenericFilter, sortBy: string, sortDir: SortDirection, skip: number, limit: number, projection: Set<string>, each: (row: GenericRow) => Promise<void>): Promise<void> {
        ...
        return new Promise<void>(function (resolve, reject) {
            ...
            stream.on("data", async function (row) {
                ...
                try {
                    busyPromise = each(normalizeSQLResults([row])[0]);
                    await busyPromise;
                } catch (ex) {
                    stream.destroy();
                    reject(ex);
    ...
    findStreamSync(table: string, filter: GenericFilter, sortBy: string, sortDir: SortDirection, skip: number, limit: number, projection: Set<string>, each: (row: any) => void): Promise<void> {
        ...
        return new Promise<void>(function (resolve, reject) {
            ...
            stream.on("data", async function (row) {
                try {
                    each(normalizeSQLResults([row])[0]);
                } catch (ex) {
                    stream.destroy();
                    reject(ex);
                }
    ...
    insert(table: string, row: GenericRow, key: string, callback?: (value: GenericKeyValue) => void): Promise<void> {
        ...
        if (key && (row[key] === null || row[key] === undefined)) {
            // Auto-genrated key
            sentence += " RETURNING `" + toSnakeCase(key) + "`";
            insertReturns = true;
        }
        ...
        return new Promise<void>(function (resolve, reject) {
            this.pool.query(sentence, values, function (error, results, fields) {
                ...
                if (insertReturns) {
                    if (results && results.length > 0) {
                        callback(results[0][toSnakeCase(key)]);
                    }
                }
                resolve();
            }.bind(this));
    ...
    update(table: string, keyName: string, keyValue: GenericKeyValue, updated: GenericRow): Promise<void> {
        ...
        for (const key of keys) {
            ...
            sentence += "`" + toSnakeCase(key) + "` = ?";
            values.push(toSQLCompatibleValue(updated[key]));
        }

        sentence += " WHERE `" + toSnakeCase(keyName) + "` = ?";
    ...
    delete(table: string, keyName: string, keyValue: GenericKeyValue): Promise<boolean> {
        const sentence = "DELETE FROM `" + table + "` WHERE `" + toSnakeCase(keyName) + "` = ?";
        const values = [keyValue];
        this.debug("[MYSQL] " + MySQL.format(sentence, values));
    ...
    sum(table: string, filter: GenericFilter, id: string, field: string): Promise<number> {
        let sentence = "SELECT SUM(`" + toSnakeCase(field) + "`) AS `" + toSnakeCase(field) + "` FROM `" + table + "`";
        const values = [];
        ...
        return new Promise<number>(function (resolve, reject) {
            this.pool.query(sentence, values, function (error, results, fields) {
                ...
                const normalized = normalizeSQLResults(results);
    ...
    increment(table: string, keyName: string, keyValue: GenericKeyValue, prop: string, inc: number): Promise<void> {
        const sentence = "UPDATE `" + table + "` SET `" + toSnakeCase(prop) + "` = `" + toSnakeCase(prop) + "` + ? WHERE `" + toSnakeCase(keyName) + "` = ?";
